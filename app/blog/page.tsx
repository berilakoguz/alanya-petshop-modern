          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {blogPosts.map((post, index) => (
              <Reveal key={post.slug} delay={index * 0.05}>
                <article className="grid h-full overflow-hidden rounded-[8px] border hairline bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lift">
                  <img src={post.image} alt={post.title} className="aspect-[16/10] h-full w-full object-cover" />
                  <div className="grid p-6">
                    ...
                    <Link href={post.href} className="mt-6 inline-flex items-center font-bold text-green-700 hover:underline">
                      Devamını Oku →
                    </Link>
                  </div>
                </article>
